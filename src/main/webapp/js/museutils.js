...
Muse.Assert.fail = function (a) {
    alert("MuseJSAssert: " + a)
};
Muse.Assert.assert = function (a, b) {
    if (!a)throw Error(b);
};
$.extend($.browser, {SafariMobile: navigator.userAgent.toLowerCase().match(/iP(hone|ad|od)/i)});
...
Muse.Utils.firstDescendant = function (a, b, c) {
    for (var d = 0; d < a.childNodes.length; d++) {
        ...
        if (g.nodeType == 1) {
            ...
            if (!c || !c.matches(g))if (g = Muse.Utils.firstDescendant(g, b, c))return g
        }
    }
    return null
};
...
Muse.Utils.getPageStyleSheet = function () {
    for (var a = 0; a < document.styleSheets.length; ++a) {
        var b = document.styleSheets[a], c = b.ownerNode ? b.ownerNode : b.owningElement;
        if (c && c.id == "pagesheet")return b
    }
};
Muse.Utils.getStyleSheetRuleById = function (a, b) {
    var c = "#" + b.toLowerCase();
    return Muse.Utils.anyStyleSheetRule(a, function (a) {
        return a.toLowerCase() == c
    })
};
Muse.Utils.anyStyleSheetRule = function (a, b) {
    var c = !1, d;
    try {
        d = a.cssRules
    } catch (g) {
    ...
    for (var f = 0; f < d.length; ++f) {
        var l = d[f];
        if (Muse.Utils.isDefined(l.selectorText))if (c) {
            if (b(l.selectorText))return l
        } else for (var h = l.selectorText.split(/\s*,\s*/), k = 0; k < h.length; k++)if (b(h[k]))return l
    }
    return null
};
Muse.Utils.getRuleProperty = function (a, b) {
    return a.style.getPropertyValue ? a.style.getPropertyValue(b) : a.style.getAttribute(b)
};
Muse.Utils.setRuleProperty = function (a, b, c) {
    a.style.setProperty ? a.style.setProperty(b, c, "") : a.style.setAttribute(b, c, 0)
};
Muse.Utils.removeRuleProperty = function (a, b) {
    a.style.removeProperty ? a.style.removeProperty(b) : a.style.removeAttribute(b, 0)
};
Muse.Utils.cloneStyleSheetRule = function (a) {
    if (!Muse.Utils.isDefined(Muse.Utils.cloneStyleSheetRule.newNumber))Muse.Utils.cloneStyleSheetRule.newNumber = 1;
    var b = Muse.Utils.getPageStyleSheet(a), c = Muse.Utils.getStyleSheetRuleById(b, a), d = "c" + Muse.Utils.cloneStyleSheetRule.newNumber++, g = "#" + d;
    b.insertRule ? b.insertRule(c.cssText.replace("#" + a, g), b.cssRules.length) : b.addRule(g, c.style.cssText);
    return d
};
...
Muse.Utils.getCanvasDirection = function (a, b) {
    var c = a.closest("*[data-rotate]"), c = c.length > 0 ? parseFloat(c.data("rotate")) % 360 : 0;
    return {
...
Muse.Utils.processHyperlink = function (a) {
    var b = a.href, c = $(window), a = $(a), d = a.attr("target");
    if (!d || d == "_self") {
        var g = b.lastIndexOf("/"), d = b.lastIndexOf("#"), f = a.attr("class").match(/anim_(\w+)/);
        if (f && d > g) {
            var a = c.data("scrollWrapper"), l = b.substring(d), d = Muse.Utils.getAnchorWithDestination(l).offset(), b = f[1], h = a || window, g = document.documentElement || document.body, f = (a ? a.scrollHeight() : g.scrollHeight) - c.height(), c = (a ? a.scrollWidth() : g.scrollWidth) - c.width(), k = Math.min(f, d.top + (a && !a.isStandard() ? a.scrollTop() :
                0)), i = Math.min(c, d.left + (a && !a.isStandard() ? a.scrollLeft() : 0)), c = function () {
                h.scrollTo(i, k);
                try {
                    history.replaceState({})
                } catch (a) {
                    if (!jQuery.browser.msie || jQuery.browser.version > 7)window.location.hash = l
                }
            };
            try {
            ...
            if (window.scrollTo || void 0 !== a) {
                var a = a || $(document), n = a.scrollLeft(), o = a.scrollTop(), p = n, r = o;
                $({scrollDistance: 0}).animate({scrollDistance: 1}, {
                    duration: 1E3, easing: b, step: function (a) {
                        a != 0 && (r = a * (k - o), p = a * (i - n), h.scrollTo(n + p, o + r))
                    }, complete: c
                })
...
Muse.Utils.redirectHyperlink = function (a) {
    if (Muse.Utils.redirectCancelled)setTimeout(function () {
        Muse.Utils.redirectCancelled = !1
    }, 0); else if (actionStack = [], Muse.Utils.processHyperlink(a) && !Muse.Utils.isIBE()) {
        var b = $(a).attr("target");
        b || (b = "_self");
...
Muse.Utils.addHyperlinkAnchor = function (a) {
    ...
    a.bind("mouseup keyup", function (a) {
        if (Muse.Utils.isMouseLeftClick(a) && actionStack.indexOf(this) != -1)a.ctrlKey || a.metaKey ? Muse.Utils.redirectHyperlinkInNewTab(this.href, a.shiftKey) : Muse.Utils.redirectHyperlink(this); else if (Muse.Utils.isMouseMiddleClick(a) && actionStack.indexOf(this) != -1)if (jQuery.browser.webkit || !a.target.href &&
            jQuery.browser.msie)Muse.Utils.redirectHyperlinkInNewTab(this.href, a.shiftKey); else return actionStack = [], !0; else Muse.Utils.isRedirectLinkKeyboardAction(a) && Muse.Utils.redirectHyperlink(this);
        return !1
    });
...
Muse.Utils.enableAnchorLinksActiveState = function () {
    var a = $("#page"), b = a.outerWidth() / a.outerHeight() > 2, c = [], a = $(window), d = Muse.Utils.getPageStyleSheet(), g = function (a) {
        ...
        c.splice(0, c.length);
        $("a.nonblock,a.block").each(function () {
            Muse.Utils.saveHyperlinkInfo($(this), g($(this)), d, b, c)
        });
        c.sort(function (a,
                         b) {
    ...
    f();
    if (0 != c.length) {
        var l = !1, h = a.data("scrollWrapper"), k = h || a, i = function () {
            l = !1;
            var a = b ? k.scrollLeft() : k.scrollTop(), d;
            a:{
                var i = 0;
                d = c.length;
                for (var j; i < d; i++)if (j = c[i], j.from <= a && a <= j.to) {
                    d = i;
                    break a
                }
                d = -1
            }
            var m, q, i = Math.max(0, d);
            for (d = Math.min(d + 2, c.length); i < d; i++)if (m = c[i], j = m.$elem.offset().left + (h && !h.isStandard() ? h.scrollLeft() : 0), q = m.$elem.offset().top + (h && !h.isStandard() ? h.scrollTop() : 0), m.from != (b ? j : q)) {
                f();
                break
            }
            i = 0;
            for (d =
                ...
                m = m.hyperLinks;
                q = void 0;
                for (var u = 0; u < m.length; u++)q = g(m[u]), j && !m[u].hasClass(q) ? m[u].addClass(q) : !j && m[u].hasClass(q) && m[u].removeClass(q)
            }
        }, j = function () {
            l || (l = !0, Muse.Utils.requestAnimationFrame(i))
        };
        (h = a.data("scrollWrapper")) ? h.registerUpdateCallback(j) : a.scroll(j);
        i()
    }
};
...
Muse.Utils.saveHyperlinkInfo = function (a, b, c, d, g) {
    var f = a.attr("href"), l = Muse.Utils.pathOnly(f), h = -1, k = a.attr("target"), i = window.location.href.replace(/#.*$/i, "");
    if (f && -1 != f.indexOf("#") && !(k && k != "_self") && !(0 <= l.indexOf("/")) && (i.charAt(i.length - 1) == "/" && (i += "index.html"), -1 != i.indexOf("/" + l, i.length - l.length - 1))) {
        var l = $(window).data("scrollWrapper"), j = f.substring(f.lastIndexOf("#")), a = a.parent('[class~="sbg"]').length > 0 || a.hasClass("block") ? a.parent() : a, n = "#" + a.attr("id"), b = "." + b;
        if (null !== Muse.Utils.anyStyleSheetRule(c,
                function (a) {
                    return 0 <= a.indexOf(n + b) || 0 <= a.indexOf(b + n)
                })) {
            k = 0;
            for (i = g.length; k < i; k++)if (g[k].href == f) {
                h = k;
                break
            }
            if (-1 == h) {
                c = Muse.Utils.getAnchorWithDestination(j);
                if (c.length === 0)return;
                k = l && !l.isStandard();
                d = Math.floor(d ? c.offset().left + (k ? l.scrollLeft() : 0) : c.offset().top + (k ? l.scrollTop() : 0));
                l = Number.MAX_VALUE;
                k = 0;
                for (i = g.length; k < i; k++)if (g[k].href != f && g[k].from == d) {
                    h = k;
                    break
                }
                if (-1 == h) {
...
Muse.Utils.needPIE = function (a) {
    if (Muse.Utils.havePIE)a(); else if (Muse.Utils.pieFunctionQueue.push(a), !Muse.Utils.pieLoading)Muse.Utils.pieLoading = !0, a = "scripts/pie.js", a[0] == "/" && (a = location.pathname.indexOf(".html") != -1 ? location.pathname.substring(0, location.pathname.lastIndexOf("/")) + a : location.pathname + a, a = a.replace(/\/+/g, "/")), $.ajax({
        url: a,
        dataType: "script",
        complete: function () {
...
Muse.Utils.transformMarkupToFixBrowserProblemsPreInit = function () {
    jQuery.browser.msie ? (jQuery("html").addClass("ie"), jQuery.browser.version < 8 && Muse.Utils.changeLItoDIVs(), jQuery.browser.version <= 8 && Muse.Utils.monitorCheckboxes()) : jQuery.browser.SafariMobile && jQuery("body").css("-webkit-text-size-adjust", "none")
};
...
Muse.Utils.transformMarkupToFixBrowserProblems = function () {
    ...
    jQuery.browser.msie && jQuery.browser.version <= 9 && (jQuery.browser.version <= 9 && (Muse.Utils.addGradientFill(), Muse.Utils.addShadows()), jQuery.browser.version < 9 && (Muse.Utils.applyIEFilterToPNGImages(), Muse.Utils.addRoundedCorners(), Muse.Utils.addRGBA(), Muse.Utils.removeEdgeAnimationBorderForIE78()), jQuery.browser.version < 8 && Muse.Utils.fixWidthsForClearingInIE7());
    (jQuery.browser.msie && jQuery.browser.version < 9 || jQuery.browser.webkit) &&
...
Muse.Utils.fixSVGImages = function () {
    var a = document.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#Image", "1.1"), b = $("html");
    a || (b.addClass("nosvg"), $("body img").each(function () {
        var a = $(this), b = a.data("mu-svgfallback");
        b && a.attr("src", b)
    }))
};
...
Muse.Utils.fixPNGImages = function () {
    $("body *").each(function () {
        var a = this, b = $(a);
        (b.css("background-image").match(/\b.png/i) || a.nodeName && a.nodeName.toLowerCase() == "img" && b.attr("src").match(/\b.png/i)) && Muse.Utils.needPIE(function () {
            b.css("-pie-png-fix", "true");
            PIE.attach(a)
        })
    })
};
        ...
        Muse.Utils.needPIE(function () {
            ...
            if (!b.data("mu-ie-matrix")) {
                ...
                if (!c || !(c.toLowerCase().indexOf("opacity") > 0 && c.indexOf("=100") < 0)) {
                    if (a.childNodes.length && !Muse.Browser.Features.checkCSSFeature("border-radius") && (c = Muse.Utils.firstChild(a)) && c.nodeName.toLowerCase() == "img") {
                        var c = $(c), d = c.attr("src"), g = b.css("background-color") + " ", f = c.css("margin-left");
                        if (f == "0px" || f == "auto")f = c.css("padding-left");
                        var l = c.css("margin-top");
                        if (l == "0px" || l == "auto")l = c.css("padding-top");
                        c.css("visibility", "hidden");
                        b.css("background", g + "url(" + d + ") no-repeat " + f + " " + l)
                    }
                    if (jQuery.browser.msie && jQuery.browser.version < 8 && (b.css("border-left-width") == 0 || b.css("border-left-style") == "none") && (b.css("border-right-width") == 0 || b.css("border-right-style") == "none") && (b.css("border-top-width") == 0 || b.css("border-top-style") == "none") && (b.css("border-bottom-width") == 0 || b.css("border-bottom-style") == "none"))b.css({
                        "border-right-width": "1px",
                        "border-right-style": "solid", "border-right-color": b.css("background-color")
                    }), b.width(b.width() - 1);
                    PIE.attach(a)
                }
            }
...
Muse.Utils.resizeHeight = function () {
    $(".browser_width").each(function () {
        var a = $(this), b = a;
        a.parent().hasClass("sbg") && (b = a.parent());
        a = $(a.children()[0]);
        a.css("position") != "fixed" && (b.height(a.outerHeight()), a.watch("height", function () {
            b.height($(this).outerHeight())
        }))
    })
};
Muse.Utils.fixWidthsForClearingInIE7 = function () {
    $(".colelem").each(function () {
        var a = $(this).offset().left - $(this).parent().offset().left;
        if ($(this).width() < 1 || $(this).width() + a < 1)$(this).css("width", (a < 0 ? 1 - a : 1) + "px")
    })
};
...
Muse.Utils.fixTransformRotations = function () {
    Muse.Browser.Features.checkCSSFeature("transform") || $("*[data-mu-ie-matrix]").each(function () {
        var a = $(this), b = a.parent(), c = Math.round(a.data("mu-ie-matrix-dx")), d = Math.round(a.data("mu-ie-matrix-dy")), g = b.innerHeight(), f = b.innerWidth();
        a.css({
            filter: function (b, c) {
        ...
        b.css({
            "margin-bottom": "-=" + (b.innerHeight() - g), "margin-right": "-=" + (b.innerWidth() -
            f)
        });
...
Muse.Utils.fullPage = function (a) {
    $(window).data("stickyFooter").init(a)
};
...
Muse.Utils.firstDefined = function () {
    for (var a = 0; a < arguments.length; a++)if (Muse.Utils.isDefined(arguments[a]))return arguments[a]
};
...
Muse.Utils.initWidget = function (a, b) {
    $(a).each(function () {
        b(this)
    })
};
Muse.Utils.showWidgetsWhenReady = function () {
    jQuery(".disn").removeClass("disn");
    jQuery(".invi").removeClass("invi");
    jQuery(".widget_invisible").removeClass("widget_invisible")
};
...
Muse.Utils.attachIframesAndObjectsToResumeMedia = function (a) {
    ...
    if (b = a.data("paused"))for (c = 0; c < b.length; c++)d = b[c], d.playFn(d.$node);
    $("iframe", a).each(function () {
        var a = $(this), b = a.attr("src"), c = a.data("src");
...
Muse.Utils.VimeoVideoHelper = function (a) {
    var b = [], c = function (a, b) {
        ...
        d && b.push({source: g, method: c, callbackFn: d});
        c = '"method": "' + c + '"';
        ...
        (!c || !c.length) && a("iframe").each(function () {
            if (this.contentWindow == b.source)return c = a(this), !1
        });
        return c
    }, f = function (a) {
        ...
        if (c) {
            if ("ready" == c.event) {
                ...
                if (d && d.length)for (var j = 0; j < d.length; j++)d[j]();
                f.data("readyQueue", null)
            }
            for (j = 0; j < b.length;)f = b[j], f.source == a.source && f.method == c.method ? (f.callbackFn(c.value), b.splice(j, 1)) : j++
        }
    };
    window.addEventListener ? window.addEventListener("message",
        f, !1) : window.attachEvent("onmessage", f, !1);
    f = function () {
    };
...
(function (a) {
    a(window);
    var b = a("html"), c = ["src"], d = ["hidpi-src", "src"], g = a(".hidpi_button"), f = function () {
        this._mode = "standard"
    };
    f.swapSources = function (a, b, c) {
        var f = a.data(b);
        f && !("src" == b && a.hasClass("ImageInclude") && "images/blank.gif" == a.attr("src") && a.parents(".SlideShowWidget").length) && ("src" == c && !a.data(c) && a.data(c, a.attr("src")), a.attr("src", f))
    };
    f.isRetina = function () {
    ...
    f.prototype.initializeHiDPIButton = function () {
        if (f.isRetina) {
            var a = this;
            g.removeClass("unavailable").click(function () {
                switch (a._mode) {
                    case "standard":
                        a.hidpiMode();
                        break;
                    case "hidpi":
                        ...
                        break;
                    default:
                        Muse.Assert.assert(!1, "Unknown mode: " + a._mode)
                }
            })
        }
    };
    f.prototype.activate = function () {
        this.initializeHiDPIButton();
        f.isRetina && (!f.shouldUseCookie || "hidpi" == f.getResolutionPreference()) ? this.hidpiMode() : this.standardMode()
    };
    ...
    f.prototype.standardMode = function () {
    ...
    f.prototype.hidpiMode = function () {
...
Muse.Utils.detectScreenResolution = function () {
    $(window).data("ResolutionManager").activate()
};
...
Muse.Browser.Features.Touch = function () {
    ...
    }; else for (var a = 0, b = Muse.Browser.domPrefixes.length; a < b; a++) {
        var c = Muse.Browser.domPrefixes[a];
        if (c + "MaxTouchPoints"in navigator && navigator[c + "MaxTouchPoints"])return c = c.toUpperCase(), {
            Start: c + "PointerDown", End: c + "PointerUp", Move: c +
            "PointerMove", Listener: function (a) {
                return function (b) {
...
Muse.Browser.Features.checkCSSFeature = function (a, b) {
    var c = Muse.Utils.toCamelCase(a), b = b || document.createElement("div");
    if (c in b.style)return !0;
    for (var c = c.charAt(0).toUpperCase() + c.substr(1), d = 0, g = Muse.Browser.domPrefixes.length; d < g; d++)if (Muse.Browser.domPrefixes[d] + c in b.style)return Muse.Browser.domPrefixes[d];
    return !1
};
Muse.Browser.Features.checkCSSValueCompatibility = function (a, b) {
    var c = document.createElement("div"), a = Muse.Utils.toCamelCase(a), d = Muse.Browser.Features.checkCSSFeature(a, c);
    if (d)d !== !0 && (a = d + a.charAt(0).toUpperCase() + a.substr(1)); else return !1;
    d = c.style[a];
    c.style[a] = b;
...
(function (a) {
    ...
    d.prototype.init = function (d) {
        this.$spacer = a(d + " .verticalspacer");
        this.$page = a(d);
        this.spacerMinHeight = 1;
        this.$spacer.css("min-height", this.spacerMinHeight);
        ...
        this.pageMarginBottom =
            Muse.Utils.getCSSIntValue(c, "padding-bottom") + Muse.Utils.getCSSIntValue(c, "margin-bottom");
        this.pageResizeWatchEnabled = !0;
        this.alwaysVertScroll = c.hasClass("always_vert_scroll");
        this.updateSpacerMargin = 0 != Muse.Utils.getCSSIntValue(this.$spacer, "margin-bottom");
        var f = this;
        if (Muse.Browser.Bugs.CannotHandleClearBoth && 0 != Muse.Utils.getCSSIntValue(this.$spacer, "margin-bottom"))this.$spacer.css("margin-bottom", 0), this.updateSpacerMargin = !1;
    ...
    d.prototype.updateScrollClass = function (a) {
        var a = this.spacerMinHeight < Math.floor(a * 100) / 100, b = !1;
        this.alwaysVertScroll || (a && !c.hasClass("no_vert_scroll") ? (c.addClass("no_vert_scroll"), b = !0) : !a && c.hasClass("no_vert_scroll") && (c.removeClass("no_vert_scroll"), b = !0));
        b && this.$spacer.css("height")
    };
    d.prototype.doUpdate = function (a) {
        if (this.enabled)if (this.initialized) {
            parseInt(a) || (a = 0);
            this.updateSpacerMargin && this.$spacer.css("margin-bottom",
                -(this.$spacer.offset().top - this.pageMarginTop));
            var c = this.$page.outerHeight(!0), d = c - this.spacerHeight, a = Math.max(this.spacerMinHeight, b.height() - this.pageMarginTop - this.pageMarginBottom - d - a);
...
Muse.Utils.requestAnimationFrame = function () {
    return window.webkitRequestAnimationFrame && window.webkitRequestAnimationFrame.bind(window) || window.mozRequestAnimationFrame && window.mozRequestAnimationFrame.bind(window) || window.requestAnimationFrame && window.requestAnimationFrame.bind(window) || function (a) {
        ...
        if (-1 != a) {
            ...
            for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
                ...
                if ("generator" == d.getAttribute("name")) {
                    "2015.0.2.310" != d.getAttribute("content") && Muse.assets.outOfDate.push("museutils.js");
                    break
                }
            }
        }
